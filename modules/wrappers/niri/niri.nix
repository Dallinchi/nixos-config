{
  ...
  perSystem = {pkgs, lib, config, ...}: let
    ...
    conf =
      ...
      binds {
        ...
        Super+D { spawn "sh" "-c" "pkill wlr-which-key || ${(lib.getExe (mkMenu [
            {
              ...
              cmd = "sh -c 'noctalia-shell ipc call launcher toggle'";
            }
            {
              key = "c";
              desc = "CLI | TUI";
        ...
        Super+V { spawn "sh" "-c" "noctalia-shell ipc call launcher clipboard"; }
  ...
  in {
    ...
    packages.niri-dev = inputs.wrappers.lib.wrapPackage {
      inherit pkgs;
      package = pkgs.niri;
