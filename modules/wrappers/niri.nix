{
  ...
  config = {
      settings = let
        startNoctaliaExe = lib.getExe self.packages.${pkgs.stdenv.hostPlatform.system}.start-noctalia-shell;
  ...
  perSystem = {pkgs, ...}: {
    packages.niri-wrapper = inputs.wrapper-modules.wrappers.niri-wrapper.wrap {
      inherit pkgs;
      imports = [self.wrappers.niri];
    };
  };
}
